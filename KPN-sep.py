...
class KPN(nn.Module):
    def __init__(self, color=True, in_channel=100,burst_length=8, blind_est=False, kernel_size=[5], sep_conv=False,
        ...
        #TODO: for now exclude the additional channel for the std estimation
        in_channel = (3 if color else 1) * (burst_length if blind_est else burst_length)
        ...
        self.kernel_pred1 = KernelConv(kernel_size, sep_conv, self.core_bias)
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        output_motion_eliminated = self.kernel_pred(data, core[:, :self.out_channel//2, ...], white_level)
        # output_denoised = self.kernel_pred1(output_motion_eliminated, core[:, self.out_channel//2:, ...], white_level)
        # return self.kernel_pred(data, core, white_level)
        return output_motion_eliminated, torch.mean(output_motion_eliminated, dim=1, keepdim=False)
        # return torch.cat([output_motion_eliminated, output_denoised], axis=1) , torch.mean(output_denoised, dim=1, keepdim=False)
